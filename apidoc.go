...
func ApiDocumentsPost(req *http.Request, w http.ResponseWriter, r render.Render, doc Document) {
	user, err := JwtGetUser(req)
	if err != nil || user.ID.Hex() == "" {
		r.JSON(http.StatusUnauthorized, nil)
		return
...
func ApiDocumentsPut(req *http.Request, w http.ResponseWriter, r render.Render, candidateDoc Document, params martini.Params) {
	user, err := JwtGetUser(req)
	if err != nil || user.ID.Hex() == "" {
		r.JSON(http.StatusUnauthorized, nil)
		return
